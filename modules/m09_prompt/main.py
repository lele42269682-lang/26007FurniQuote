...
logger = get_module_logger("M09_PROMPT")



@isolated(module_id="m09_prompt")
def run(payload: dict) -> dict:
    """模块入口（占位）。开发时替换为实际业务逻辑。"""
    log_input(logger, payload)
    log_error(logger, "模块 m09_prompt 尚未实现，仅占位")
    result = {"status": "not_implemented", "module": "m09_prompt"}
...
if __name__ == "__main__":
    # 自测入口（热拔插验证）
    import json
    sample_path = "modules/m09_prompt/test_data/sample_input.json"
    try:
